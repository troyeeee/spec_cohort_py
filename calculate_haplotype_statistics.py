...
def parse_vcf_phase(vcf_file, CHROM, indels = False):
    ...
    # the first open operation here is only for checking whether we have phase set or not
    ...
    not_considered_pos_list = []
    not_considered_indel_count = 0
    not_considered_hom_count = 0
    with open(vcf_file, 'r') as vcf:
        ...
        for line in vcf:
            ...
            if not (len(genotype) == 3 and genotype[0] in ['0','1','2'] and
                ...
                not_considered_pos_list.append(el[1])
                consider = False

            # filtered hom sites
            if genotype[0] == genotype[2]:
                not_considered_hom_count += 1
                consider = False

            # filtered indel sites
            if consider and (not indels) and (('0' in genotype and len(a0) != 1) or
                                              ('1' in genotype and len(a1) != 1) or ('2' in genotype and len(a2) != 1)):
                # print("not considered, no indels", el)
                not_considered_indel_count+=1
                consider = False
            ...
            if PS_index == None:
                ...
                print("ps index not null, no considering")
                ps = dat[PS_index]
                if ps == '.':
                    consider = False
            ...
            if ps != None and consider and phase_data[1] == '|':
                ...
                considered_count+=1
                considered_pos_list.append(el[1])
            snp_ix += 1

    print("considered count = {}, not considered unphased count = {}, not considered indel count = {}, not considered hom count = {}".format(considered_count, not_considered_count, not_considered_indel_count, not_considered_hom_count))
    print("not considered pos = ", not_considered_pos_list)
    # print("considered pos list = ", considered_pos_list)
