...
def parse_args():
    ...
    parser.add_argument('-l', '--log', nargs='+',type = str, help='Use this flag to consider indel variants. Default: Indels ignored.',default="./log.tsv")
...
switch err loc:     {}
mismatch error loc: {}
            '''.format(self.switch_count, self.get_switch_rate(), self.get_mismatch_count(), self.get_mismatch_rate(),
                       self.get_flat_error_rate(), self.get_phased_count(),
                       self.get_AN50(), self.get_N50_phased_portion(), sum(self.maxblk_snps.values()), self.switch_loc, self.mismatch_loc))
...
def vcf_vcf_error_rate_multiple(assembled_vcf_files, reference_vcf_files, indels, largest_blk_only=False):
    ...
    err = error_result()
    print(assembled_vcf_files, reference_vcf_files)
    print(zip(assembled_vcf_files, reference_vcf_files))
    total_err_dict = vcf_vcf_error_rate(assembled_vcf_files[0], reference_vcf_files[0], indels)
    return total_err_dict

def parse_vcf_phase_to_dict(vcf_file, CHROM, indels = False):
    ...
    blocks_dict = {}
    for s in samples:
        samples_snp_count[s] = 0
        samples_blocks[s] = defaultdict(list)
    # print(samples)
    for s in samples:
        for rec in bcf_in.fetch():

            consider = True
            phase_data = rec.samples[s]['GT']
            a0 = rec.ref # str
            a1 = rec.alts # tuple
            a2 = None
            # print(phase_data)
            ...
            if not (len(phase_data)==2 and  phase_data[0] in [0,1,2] and phase_data[1] in [0,1,2] and \
                rec.samples[s].phased):
                consider = False
            if phase_data[0] == phase_data[1]:
                consider = False
            # print(phase_data, a0,a1,a2)
            # print(type(a1))
            if isinstance(a1,tuple):
                # print('tuple')
                a1 = a1[0]
            if consider and (not indels) and ((0 in phase_data and len(a0)!=1) or \
                (1 in phase_data and len(a1)!=1) or (2 in phase_data and len(a2) !=1)):
                # print("yes")
                # print(a0,a1,a2)
            ...
            if ps != None and consider and rec.samples[s].phased:
                samples_blocks[s][ps].append((samples_snp_count[s], pos, phase_data[0], phase_data[1], a0, a1, a2))
                # print(samples_blocks[s])
            samples_snp_count[s] += 1
            
        blocks_dict[s] = [v for k,v in sorted(list(samples_blocks[s].items())) if len(v) > 1]
        print("block count: ", len(samples_blocks[s][ps]))
    return blocks_dict, samples_snp_count
...
def vcf_vcf_error_rate(assembled_vcf_file, reference_vcf_file, indels, largest_blk_only=False):
    ...
    t_blocklist_dict = {"sample":[]}
    t_blocklist_dict, _ = parse_vcf_phase_to_dict(reference_vcf_file, CHROM, indels)
    print("phased vcf file loading")
    a_blocklist_dict, samples_snp_count = parse_vcf_phase_to_dict(assembled_vcf_file, CHROM, indels)
    total_errs = {}
    n_samples = len(t_blocklist_dict)
    print("parsed blocks")
    for k, v in t_blocklist_dict.items():
        t_blocklist = t_blocklist_dict[k]
        a_blocklist = a_blocklist_dict[k]
    # print("template vcf file loading")
    # t_blocklist = parse_vcf_phase(reference_vcf_file, CHROM, indels)
    # print("phased vcf file loading")
    # a_blocklist = parse_vcf_phase(assembled_vcf_file, CHROM, indels)
    # print(len(a_blocklist))
    # print(len(a_blocklist[0]))
    # compute error result object
        if largest_blk_only:
            largest_blk = []
            for blk in a_blocklist:
                if len(blk) > len(largest_blk):
                    largest_blk = blk
            a_blocklist = [largest_blk]
        print("calculating the error snp counts and rate for a_blocklist")
        # todo::calculate numsnps for each sample
        num_snps = samples_snp_count[k]


        err = error_rate_calc_v2(t_blocklist, a_blocklist, CHROM, num_snps, indels)
        print("pass....")
        total_errs[k] = err
    return total_errs
...
def error_rate_calc_v2(t_blocklist, a_blocklist, ref_name, num_snps,  indels=False, phase_set=None):
            ...
            for a in [0,1]: # choose which allele to score. this only makes a difference for minimizing switch errors vs mismatches in corner cases.
                ...
                for blk_ix, (snp_ix, pos, a1, a2,  ref_str, alt1_str, alt2_str) in enumerate(a_block):
                    ...
                    if (x != y and not switched) or (x == y and switched): # current base is mismatched, implying a switch
                        ...
                        if (a == 0) :
                            flip_0_count+=1
                            flip_0_loc.append(pos)
                        else:
                            flip_1_count+=1
                            flip_1_loc.append(pos)
    ...
    if flip_0_count > flip_1_count:
        print("flip error count = ", flip_1_count)
        print("flip error locations = ", flip_1_loc)
    else:
...
if __name__ == '__main__':
    ...
    if args.haplotype_blocks1 == None and args.haplotype_blocks2 == None:
        print(args.vcf1, args.vcf2, args.indels)
        total_err_dict =  vcf_vcf_error_rate_multiple(args.vcf1, args.vcf2, args.indels)
        total_mer = 0.0
        total_ser = 0.0
        n_sample = len(total_err_dict)
        out_file = open(args.log[0], 'w')
        tsv_writer = csv.writer(out_file, delimiter='\t')
        tsv_writer.writerow(["", "switch rate", "mismatch rate", "total rate"])
        for k, v in total_err_dict.items():
            tsv_writer.writerow([k,v.get_switch_rate(), v.get_mismatch_rate(), v.get_switch_rate()+v.get_mismatch_rate() ])
            total_mer += v.get_switch_rate()
            total_ser += v.get_mismatch_rate()
        tsv_writer.writerow(["total: ", total_ser, total_mer, total_ser + total_mer])
        tsv_writer.writerow(["mean: ", total_ser/n_sample, total_mer/n_sample, (total_ser + total_mer)/n_sample])
        print(total_ser, total_mer)
